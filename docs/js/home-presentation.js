...
/** Demo JS function used by the Back→Front mock (no live Glue bridge on Pages). */
...
(function () {
  ...
  if (versionEl) {
    versionEl.textContent =
      'Glue v' + (window.GLUE_SITE && window.GLUE_SITE.version ? window.GLUE_SITE.version : '0.6.6');
  }
  ...
  /* —— Scroll reveals —— */
  function hideStoryReveals() {
    gsap.utils.toArray('.story-scroll [data-reveal]').forEach((el) => {
      gsap.killTweensOf(el);
      gsap.set(el, { opacity: 0, y: 48, force3D: true, clearProps: '' });
    });
  }

  function killStoryRevealTriggers() {
    ScrollTrigger.getAll().forEach((t) => {
      if (t.vars && String(t.vars.id || '').indexOf('story-reveal') === 0) {
        t.kill();
      }
    });
  }
  ...
  function refreshReveals() {
    ...
    gsap.utils.toArray('.story-scroll [data-reveal]').forEach((el, i) => {
    ...
    // Let layout settle, then refresh so already-in-view panels (pitch / steps)
    // enter cleanly instead of flashing then vanishing.
    requestAnimationFrame(() => {
  ...
  async function mockBridgeEcho(payload) {
    await sleep(180);
    const message = payload && payload.message;
    return { echo: message, from: 'python', at: new Date().toISOString() };
  }

  async function mockBridgeCallJs(payload) {
    await sleep(180);
    return { js_reply: bridge_on_js(payload), from: 'python' };
  }
  ...
  async function runFrontToBack() {
    ...
    let result;
    result = await mockBridgeEcho({ message });
    setLog(logPy, result);
    await sleep(STEP_PAUSE_MS);
  ...
  async function runBackToFront() {
    ...
    setLabel(label, 'running');
    let result;
    result = await mockBridgeCallJs({ message });
